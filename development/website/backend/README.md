...
📊 Carte complète des modules : [`docs/backend-architecture-detailed.html`](../docs/backend-architecture-detailed.html) (ou la [version simplifiée](../docs/backend-architecture.html)).

Organisation en couches, du plus interne au plus externe :

| Dossier / fichier | Rôle |
|---|---|
| [`domain/`](app/domain/) | logique métier pure (aucune dépendance à FastAPI) |
| [`nao/`](app/nao/) | abstraction pour piloter NAO plus tard |
| [`auth.py`](app/auth.py) | jetons de session signés |
| [`schemas.py`](app/schemas.py) | formats des requêtes/réponses HTTP (Pydantic) |
| [`api/`](app/api/) | routes REST + WebSocket (branche le domaine au monde extérieur) |
| [`state.py`](app/state.py) | instances uniques partagées par toute l'app |
| [`main.py`](app/main.py) | point d'entrée FastAPI |
...
## [`domain/roles.py`](app/domain/roles.py) — le catalogue des rôles
...
## [`domain/models.py`](app/domain/models.py) — les entités

📊 Schéma des entités et de leurs relations : [`docs/backend-er.html`](../docs/backend-er.html).
...
## [`domain/errors.py`](app/domain/errors.py) — les erreurs métier

Une hiérarchie d'exceptions Python (`GameNotFoundError`, `PseudoTakenError`, `NotHostError`, `InvalidGameStateError`, `InvalidRolesConfigError`, `InvalidPseudoError`, `PlayerNotFoundError`) toutes héritant de `DomainError`. Le domaine lève ces erreurs sans savoir qu'elles finiront en code HTTP — c'est [`api/games.py`](app/api/games.py) qui fait la traduction.

## [`domain/service.py`](app/domain/service.py) — le cœur : `GameService`

📊 Cycle de vie d'une partie (statuts et gardes) : [`docs/backend-state.html`](../docs/backend-state.html).
...
5. **`get_state(code)`** / **`get_player(code, player_id)`** / **`require_host(code, caller_id)`** — lectures et vérification d'autorisation, utilisées par [`api/deps.py`](app/api/deps.py).
...
## [`nao/controller.py`](app/nao/controller.py) — l'abstraction NAO
...
## [`auth.py`](app/auth.py) — les jetons de session
...
## [`schemas.py`](app/schemas.py) — les formats HTTP (Pydantic)
...
## [`api/deps.py`](app/api/deps.py) — les dépendances d'authentification
...
## [`api/games.py`](app/api/games.py) — les routes REST
...
## [`ws_manager.py`](app/ws_manager.py) + [`api/ws.py`](app/api/ws.py) — le temps réel du lobby
...
## [`state.py`](app/state.py) — le câblage
...
## [`main.py`](app/main.py) — le point d'entrée
...
- **[`test_domain.py`](tests/test_domain.py)** — teste `GameService` directement, sans passer par HTTP : une partie créée n'a aucun joueur, `join_game` n'ajoute jamais l'hôte, répartition des rôles, rejet des actions par un non-hôte, erreurs si la config ne correspond pas au nombre de joueurs, tirage correct (uniquement parmi les joueurs), impossibilité de rejoindre après démarrage.
- **[`test_auth.py`](tests/test_auth.py)** — vérifie que le jeton survit à un aller-retour et qu'un jeton trafiqué est rejeté.
- **[`test_api.py`](tests/test_api.py)** — teste les routes HTTP via `TestClient` : l'hôte n'apparaît pas dans le lobby, isolation des rôles (chaque joueur ne voit que le sien), 403 pour un joueur qui tente une action d'hôte, 404 pour l'hôte qui appelle `/me`, 401 sans jeton valide.

[`conftest.py`](conftest.py) ajoute le dossier `backend/` au `sys.path` pour que `import app...` fonctionne depuis les tests.

## Flux complet, de bout en bout

📊 Version diagramme (séquence) : [`docs/backend-sequence.html`](../docs/backend-sequence.html).
